...
const products: Product[] = [
  ...
  },
  {
    ...
    description: 'A DAO based community platform for revenue sharing agreements',
    upvotes: 5,
    downvotes: 33,
    comments: 24,
...
const Home: React.FC = () => {
  return (
    <div className="relative min-h-screen w-full overflow-hidden bg-[#050505] text-white">
      ...
      {/* <div className="absolute inset-0 backdrop-blur-[74px]"></div> */}
        <h1 className=" text-4xl font-semibold mb-2 p-4 text-center">Maybe the real DApps were friends we made all along</h1>
        <p className="text-gray-400 mb-6 text-center">Hottest Web3 product directory you can't afford to miss</p>

        </div>

            
        <div className="bg-black text-white p-4 rounded-3xl max-w-md mx-auto">
      ...
      <h2 className="text-xl font-semibold mb-4">Hottest products today 🔥</h2>

      {products.map((product, index) => (
        <div key={index} className="bg-gray-900 rounded-lg p-4 mb-4">
          <div className="flex items-center mb-2">
            <div className={`w-10 h-10 bg-gradient-to-br ${product.logoColor} rounded-full mr-3`}></div>
            <div>
              <h3 className="font-semibold">{product.name}</h3>
              <div className="flex space-x-2">
                {product.categories.map((category, catIndex) => (
                  <span key={catIndex} className="text-xs px-2 py-1 rounded-full" style={{
                    backgroundColor: catIndex === 0 ? '#6b46c1' : '#10b981',
                    color: 'white'
                  }}>
                    {category}
                  </span>
          ...
          <div className="flex space-x-4 text-xs text-gray-400">
            ...
            <span className="flex items-center">
              <ThumbsDown className="w-3 h-3 mr-1" /> {product.downvotes} Downvotes
            </span>
            <span className="flex items-center">
              <MessageSquare className="w-3 h-3 mr-1" /> {product.comments} comments
            </span>
          </div>
        </div>
      ))}
